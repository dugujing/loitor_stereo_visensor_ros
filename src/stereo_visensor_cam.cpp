...
#include "loitor_stereo_visensor_ros/stereo_visensor_cam.h"
...
void StereoVisensorCam::img_data_stream ( )
{
  ...
  while(cam_n.ok())
  {
	...
	*/
	double  tmp =  floor(left_timestamp);
	msg_time.sec=(__time_t) tmp;
	msg_time.nsec=(__time_t)((left_timestamp - tmp)* 1e+9);
	...
	*/
	...
	double  tmp =  floor(right_timestamp);
	msg_time.sec=(__time_t) tmp;
	msg_time.nsec=(__time_t)((right_timestamp - tmp)* 1e+9);
